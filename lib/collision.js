...
function extensions(parentClass) {
	return {
		...
		_maybeAddLayerToRBush: function _maybeAddLayerToRBush(layer) {
		...
		_getIconBox: function _getIconBox(el) {
			...
			return [parseInt(styles.marginLeft), parseInt(styles.marginTop), parseInt(styles.marginLeft) + parseInt(styles.width), parseInt(styles.marginTop) + parseInt(styles.height)];
		},

		// Much like _getIconBox, but works for positioned HTML elements, based on offsetWidth/offsetHeight.
		_getRelativeBoxes: function _getRelativeBoxes(els, baseBox) {
			var boxes = [];
		...
		_positionBox: function _positionBox(offset, box) {
...
L.layerGroup.collision = function (options) {
	var col = new L.LayerGroup.Collision(options || {});
	console.log(col);
	return col;
};